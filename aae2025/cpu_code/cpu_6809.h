...
public:
	...
	struct m6809_Regs
	{
		...
		uint16_t	u = 0, s = 0;	/* Stack pointers */
		uint16_t	x = 0, y = 0;	/* Index registers */
		uint8_t		dp = 0;		/* Direct Page register */
		uint8_t		a = 0, b = 0;	/* Accumulator */
		uint8_t		cc = 0;     // Flags
...
private:
	...
	// Internal registers
	uint8_t opcode = 0;
	int clockticks6809 = 0;
	// uint32_t dwElapsedTicks;

	// Help variables
	int pending_interrupts = 0;	/* NS 970908 */
	int clocktickstotal = 0; // Runnning, resetable total of clockticks
	int m6809_ICount = 0;
	bool debug = false;
	bool mmem = false;         // Use mame style memory handling, reject unhandled read/writes
	bool log_debug_rw = false; // Log unhandled reads and writes
	bool irg_pending = false;

	uint8_t cc = 0, dpreg = 0, areg = 0, breg = 0;
	uint16_t xreg = 0, yreg = 0, ureg = 0, sreg = 0, pcreg = 0;

	uint8_t ireg = 0;
	uint8_t ireg2 = 0;

	uint16_t eaddr = 0; /* effective address */
