...
#include <vector>   // put near the top of vector_fonts.cpp
...
void fprint(float x, int y, unsigned int color, float scale, const char* fmt, ...)
{
	...
	len = static_cast<int>(strnlen(text, 255));
	lastscale = scale;

	Vec2 center = { 580 , 325 }; // Or use { xcenter, ycenter };
	...
	for (i = 0; i < len; i++)
	{
		...
		while (fontdata[a] != EOC)
		{
			if (fangle)
			{
				p0 = RotateAroundPoint((fontdata[a] * scale) + x,
					(fontdata[b] * scale) + y,
					center.x, center.y, cosTheta, sinTheta);

				p1 = RotateAroundPoint((fontdata[a + 2] * scale) + x,
					(fontdata[b + 2] * scale) + y,
					center.x, center.y, cosTheta, sinTheta);
			}
			else
			{
			...
			verts.push_back(p0.x);
			verts.push_back(p0.y);
		...
		if (!verts.empty())
		{
			...
			/* draw the endpoints (same vertices, GL_POINTS) */
...
void fprintc(int y, unsigned int color, float scale, const char* string)
{
	...
	float total = 0;
	float x = 1024 / 2;
	len = static_cast<int>(strnlen(string, 255));
