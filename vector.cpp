...
void draw_color_vectors()
{
 ...
 // wrlog("Drawing lines this frame %d Line Num %d max line",glines[x].sx,MAX_LINES);
  while (glines[x].sx!=LINE_END && x < MAX_LINES)
  {
	...
	if (color !=-1){
	if (color!=lastcolor){  glines[x].r+=gc;glines[x].g+=gc;glines[x].b+=gc;
	                        //Clip color values		
...
void cache_line(float startx, float starty, float endx, float endy, int zvalue, float gc, float mod)
{
 ...
 if (dvglnptr > 144900) {wrlog("possible Line Cache overflow!");cache_clear();	}
    ...
    //if (startx==-5000)
	//{  linecache[dvglnptr]=-5000;dvglnptr=0;}
	//else{
...
{ 
	...
	//if (pointx==-5000)
	//{  pointcache[dvgppntptr]=-5000;dvgppntptr=0;}
	//else{
...
{ 
	...
	//if (pointx==-5000)
	//    {  texcache[dvgtexptr]=-5000;dvgtexptr=0;}
...
void cache_end(void)
{
 ...
 if (ppntr < MIN_LINES) {ppntr=oldppntr;gpoints[ppntr].sx=LINE_END;} else {gpoints[ppntr].sx=LINE_END;ppntr=0;}
 if (lpntr < MIN_LINES) {lpntr=oldlpntr;glines[lpntr].sx=LINE_END;} else {glines[lpntr].sx=LINE_END;lpntr=0;}
...
linecache[dvglnptr]=-5000;
pointcache[dvgpntptr]=-5000;
texcache[dvgtexptr]=-5000;
