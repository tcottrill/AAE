...
char *my_get_config_string(char *section, char *name, char *def) 
{
	...
	if (is_override){
	     ...
	     GetPrivateProfileString(section,name,def,buffer,256,aaepath); 
	      if (strcmp(buffer,def)==0)return def;
	       else return buffer;
	    }
...
double my_get_config_float(char *section, char *name , double def)
{
	float val=-1;
    char buffer[64];
...
void setup_config(void)
{
	...
	config.artwork=my_get_config_int("main", "artwork" , 1);
    config.artcrop=my_get_config_int("main", "artcrop" , 1);
	config.overlay = my_get_config_int("main", "overlay" , 1);
