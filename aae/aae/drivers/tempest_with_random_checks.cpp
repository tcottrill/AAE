...
#define BIN8 "%c%c%c%c%c%c%c%c"
#define BITS(b) ((b)&0x80?'1':'0'),((b)&0x40?'1':'0'),((b)&0x20?'1':'0'),((b)&0x10?'1':'0'), \
...
READ_HANDLER(pokey_1_tempest_read)
{
	uint8_t val = 0;
	...
	if (address == 0x0a)
	{
		...
		LOG_INFO("Pokey 1 Read: Addr %x data:%x Cycles: %x", m_cpu_6502[0]->get_pc(), val, get_exact_cyclecount(0));
	}
	else
	{
		val = pokey1_r(address);
...
READ_HANDLER(pokey_2_tempest_read)
{
	...
	if ((address & 0xf) == 0x0a)
	{
		LOG_INFO("Pokey 2 Read: Addr %x data:%x Cycles: %x", m_cpu_6502[0]->get_pc(), val, get_exact_cyclecount(0));
	}
...
MEM_ADDR(0x6060, 0x6060, MathboxLowbitRead)
MEM_ADDR(0x6070, 0x6070, MathboxHighbitRead)
MEM_ADDR(0x60c0, 0x60cf, pokey_1_tempest_read)
MEM_ADDR(0x60d0, 0x60df, pokey_2_tempest_read)
...
void run_tempest()
{
	...
	if (pcheck == 100)
	{
		// Inject POKEY protection self-test routine at $7000 (ends in RTS).
		static const unsigned char prot[] = {
			0x78, 0xAD,0xCA,0x60, 0xAC,0xCA,0x60, 0x84,0x29,
			0x4A,0x4A,0x4A,0x4A, 0x45,0x29, 0x85,0x29,
			0xAD,0xDA,0x60, 0xAC,0xDA,0x60, 0x58,
			0x45,0x29, 0x29,0xF0, 0x45,0x29, 0x85,0x29,
			0x98, 0x0A,0x0A,0x0A,0x0A, 0x45,0x29,
			0x8D,0x1F,0x01, 0x60
		};
		for (int i = 0; i < (int)sizeof(prot); ++i)
			Machine->memory_region[CPU0][0x7000 + i] = prot[i];


		auto* cpu = m_cpu_6502[0];
		uint16_t sPC = cpu->get_pc();
		uint8_t  sA = cpu->A, sX = cpu->X, sY = cpu->Y, sS = cpu->S, sP = cpu->P;
		uint8_t  s29 = Machine->memory_region[CPU0][0x0029];   // routine clobbers $29
		uint8_t  s11f = Machine->memory_region[CPU0][0x011F];

		cpu->P |= 0x04;             // <-- SET I: stop step6502 servicing an IRQ mid-call
		cpu->set_pc(0x7000);
		for (int n = 0; n < 64 && cpu->get_pc() >= 0x7000 && cpu->get_pc() < 0x702A; ++n)
			cpu->step6502();        // '< 0x702A' = never run the RTS (no stack touch)

		LOG_INFO("POKEY prot $011F = %02X (0=pass)", Machine->memory_region[CPU0][0x011F]);

		cpu->set_pc(sPC); cpu->A = sA; cpu->X = sX; cpu->Y = sY; cpu->S = sS; cpu->P = sP;
		Machine->memory_region[CPU0][0x0029] = s29;             // restore scratch
		Machine->memory_region[CPU0][0x011F] = s11f;
		pcheck = 0;
			
		static const unsigned char prot2[] = {
		...
		for (int i = 0; i < (int)sizeof(prot2); ++i)
			Machine->memory_region[CPU0][0x7000 + i] = prot2[i];
...
int init_tempest(void)
{
	...
	if (config.hack)
	{
		...
		// INFINITE LIVES HACK: NOP out the "DEC $48,X" at $C9B5 -- the single
		// instruction that decrements the current player's life count on death
		// ("Decrease player lives by one" in the disassembly). With it gone the
		// life counter never drops, so the player always respawns and can never
		// game-over. (The death/respawn animation still plays.)
		//Machine->memory_region[CPU0][0xc9b5] = 0xea;
		//Machine->memory_region[CPU0][0xc9b6] = 0xea;
	}
	/*
	unsigned char* MEM = memory_region(CPU0);

	// --- POKEY protection self-test @ $7000 (mirrors game producer $AE1F-$AE46) ---
	MEM[0x7000] = 0x78;             // SEI
	MEM[0x7001] = 0xAD;             // LDA $60CA   ; pokey1 RANDOM
	MEM[0x7002] = 0xCA;
	MEM[0x7003] = 0x60;
	MEM[0x7004] = 0xAC;             // LDY $60CA   ; pokey1 RANDOM again
	MEM[0x7005] = 0xCA;
	MEM[0x7006] = 0x60;
	MEM[0x7007] = 0x84;             // STY $29
	MEM[0x7008] = 0x29;
	MEM[0x7009] = 0x4A;             // LSR
	MEM[0x700A] = 0x4A;             // LSR
	MEM[0x700B] = 0x4A;             // LSR
	MEM[0x700C] = 0x4A;             // LSR
	MEM[0x700D] = 0x45;             // EOR $29
	MEM[0x700E] = 0x29;
	MEM[0x700F] = 0x85;             // STA $29
	MEM[0x7010] = 0x29;
	MEM[0x7011] = 0xAD;             // LDA $60DA   ; pokey2 RANDOM
	MEM[0x7012] = 0xDA;
	MEM[0x7013] = 0x60;
	MEM[0x7014] = 0xAC;             // LDY $60DA   ; pokey2 RANDOM again
	MEM[0x7015] = 0xDA;
	MEM[0x7016] = 0x60;
	MEM[0x7017] = 0x58;             // CLI
	MEM[0x7018] = 0x45;             // EOR $29
	MEM[0x7019] = 0x29;
	MEM[0x701A] = 0x29;             // AND #$F0
	MEM[0x701B] = 0xF0;
	MEM[0x701C] = 0x45;             // EOR $29
	MEM[0x701D] = 0x29;
	MEM[0x701E] = 0x85;             // STA $29
	MEM[0x701F] = 0x29;
	MEM[0x7020] = 0x98;             // TYA
	MEM[0x7021] = 0x0A;             // ASL
	MEM[0x7022] = 0x0A;             // ASL
	MEM[0x7023] = 0x0A;             // ASL
	MEM[0x7024] = 0x0A;             // ASL
	MEM[0x7025] = 0x45;             // EOR $29
	MEM[0x7026] = 0x29;
	MEM[0x7027] = 0x8D;             // STA $011F   ; result: 0 = POKEY good
	MEM[0x7028] = 0x1F;
	MEM[0x7029] = 0x01;
	MEM[0x702A] = 0x60;             // RTS

	*/
	//Machine->memory_region[CPU0][0xAE19] = 0xEA; 
	//Machine->memory_region[CPU0][0xAE1A] = 0xEA;
	//Machine->memory_region[CPU0][0xAE1B] = 0xEA;
	avg_start_tempest();
...
void run_tempest()
{
	...
	if (pcheck == 100)
	{
		// Protection self-test @ $7000, now also CAPTURES the 4 raw reads to $7050-$7053.
		// (Captures are stored AFTER each read pair, so r1/r2 stay 4 cycles apart.)
		static const unsigned char prot[] = {
			0x78,                   // SEI
			0xAD,0xCA,0x60,         // LDA $60CA   ; r1
			0xAC,0xCA,0x60,         // LDY $60CA   ; r2
			0x8D,0x50,0x70,         // STA $7050   ; save r1
			0x8C,0x51,0x70,         // STY $7051   ; save r2
			0x84,0x29,              // STY $29
			0x4A,0x4A,0x4A,0x4A,    // LSR x4
			0x45,0x29,              // EOR $29
			0x85,0x29,              // STA $29
			0xAD,0xDA,0x60,         // LDA $60DA   ; a1
			0xAC,0xDA,0x60,         // LDY $60DA   ; a2
			0x8D,0x52,0x70,         // STA $7052   ; save a1
			0x8C,0x53,0x70,         // STY $7053   ; save a2
			0xEA,                   // NOP (was CLI; keep IRQs masked)
			0x45,0x29,              // EOR $29
			0x29,0xF0,              // AND #$F0
			0x45,0x29,              // EOR $29
			0x85,0x29,              // STA $29
		...
		uint16_t sPC = cpu->get_pc();
		uint8_t sA = cpu->A, sX = cpu->X, sY = cpu->Y, sS = cpu->S, sP = cpu->P;
		uint8_t s29 = Machine->memory_region[CPU0][0x0029];
		uint8_t s11f = Machine->memory_region[CPU0][0x011F];

		cpu->P |= 0x04;                 // mask IRQ
		cpu->set_pc(0x7000);
		for (int n = 0; n < 80 && cpu->get_pc() >= 0x7000 && cpu->get_pc() < 0x7036; ++n)
			cpu->step6502();            // stop before RTS at $7036

		uint8_t r1 = Machine->memory_region[CPU0][0x7050];
		uint8_t r2 = Machine->memory_region[CPU0][0x7051];
		uint8_t a1 = Machine->memory_region[CPU0][0x7052];
		uint8_t a2 = Machine->memory_region[CPU0][0x7053];
		uint8_t prot011f = Machine->memory_region[CPU0][0x011F];

		LOG_INFO("P1 r1=%02X(" BIN8 ") r2=%02X(" BIN8 ")  hi(r1)==lo(r2)? %d",
			r1, BITS(r1), r2, BITS(r2), (r1 >> 4) == (r2 & 0x0F));
		LOG_INFO("P2 a1=%02X(" BIN8 ") a2=%02X(" BIN8 ")  hi(a1)==lo(a2)? %d",
			a1, BITS(a1), a2, BITS(a2), (a1 >> 4) == (a2 & 0x0F));
		LOG_INFO("$011F = %02X (0=pass)", prot011f);

		cpu->set_pc(sPC); cpu->A = sA; cpu->X = sX; cpu->Y = sY; cpu->S = sS; cpu->P = sP;
		Machine->memory_region[CPU0][0x0029] = s29;
		Machine->memory_region[CPU0][0x011F] = s11f;
		pcheck = 0;
	}
