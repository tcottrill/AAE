...
UINT16 get_ccpu_inputs(int offset)
{
	...
	case CCPU_INPUTS::SPEEDFRK: {
		...
		else if (offset > 0x06) return (readinputportbytag("INPUTS") >> offset) & 1;
		break;
	}

	case CCPU_INPUTS::QB3: {
		/* QB3 uses input offset 0x0F as a frame position indicator.
		   All other inputs are read normally from the INPUTS port. */
		if (offset == 0x0f) return ccpu_qb3_frame_r();
		else return (readinputportbytag("INPUTS") >> offset) & 1;
		break;
	}
...
int init_qb3()
{
	...
	/* QB3 uses a custom input handler that provides a frame position
	   indicator on input offset 0x0F */
	ccpuinput_type = CCPU_INPUTS::QB3;
	return 1;
}
...
ART_LOAD("sundance.zip", "sover.png", ART_TEX, 1)
ART_END
...
ART_LOAD("armora.zip", "armorattackmaster.png", ART_TEX, 1)
ART_LOAD("armora.zip", "armor_attack_bezel.png", ART_TEX, 3)
ART_LOAD("custom.zip", "cineshot.png", GAME_TEX, 0)
ART_END
...
ART_LOAD("solarq_aae.zip", "solarquest_backdrop.png", ART_TEX, 0)
ART_LOAD("solarq_aae.zip", "solarquest_overlay.png", ART_TEX, 1)
ART_LOAD("solarq_aae.zip", "solarquest_bezel.png", ART_TEX, 3)
ART_LOAD("solarq_aae.zip", "cineshot.png", GAME_TEX, 0)
ART_END

ART_START(starcas_art)
ART_LOAD("starcas_aae.zip", "starcas.png", ART_TEX, 1)
ART_LOAD("starcas_aae.zip", "mystarcastle_bezel.png", ART_TEX, 3)
ART_LOAD("starcas_aae.zip", "cineshot.png", GAME_TEX, 0)
ART_END

ART_START(wotw_art)
ART_LOAD("wotw.zip", "wotw_overlay.png", ART_TEX, 1)
ART_LOAD("wotw.zip", "wotw.png", ART_TEX, 3)
ART_END
...
AAE_DRIVER_END()
	...
	AAE_DRIVER_CPUS(AAE_CPU_ENTRY(CPU_CCPU, 4980750, 1, 1, 0, 0, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr),
		...
		AAE_CPU_NONE_ENTRY(),
		AAE_CPU_NONE_ENTRY())
	AAE_DRIVER_VIDEO_CORE(38, 0, VIDEO_TYPE_VECTOR | VECTOR_USES_BW | VECTOR_USES_OVERLAY2, ORIENTATION_DEFAULT | ORIENTATION_FLIP_Y)
	AAE_DRIVER_SCREEN(1024, 768, 0, 1024, 0, 768)
	AAE_DRIVER_RASTER_NONE()
	AAE_DRIVER_HISCORE_NONE()
	AAE_DRIVER_VECTORRAM(0, 0)
...
AAE_DRIVER_CPUS(AAE_CPU_ENTRY(CPU_CCPU, 4980750, 1, 1, 0, 0, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr),
	...
	AAE_CPU_NONE_ENTRY(),
	AAE_CPU_NONE_ENTRY())
		AAE_DRIVER_VIDEO_CORE(38, 0, VIDEO_TYPE_VECTOR | VECTOR_USES_COLOR | VECTOR_USES_OVERLAY2, ORIENTATION_DEFAULT | ORIENTATION_FLIP_Y)
		AAE_DRIVER_SCREEN(1024, 768, 0, 1024, 0, 768)
		AAE_DRIVER_RASTER_NONE()
		AAE_DRIVER_HISCORE_NONE()
		AAE_DRIVER_VECTORRAM(0, 0)
