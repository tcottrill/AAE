...
#include "z80fmly.h"
#include "mixer.h"     // stream_start / stream_update / mixer_alloc_channel (CTC tones)
#include "config.h"    // config.samplerate
...
// ===========================================================================
// CTC square-wave tone output (CTC channels 1 & 2 -> fire/thrust tones)
//
// Modeled on phoenix_audio.cpp: one custom mixer stream holds all generated
// sound. Each frame we render both CTC tone channels' zero-count square waves
// into an internal buffer, resample it to the mixer rate, and push it. A tone
// channel that is actively timing (z80ctc_getperiod in the audible band) toggles
// every 'period' seconds; otherwise it is silent. Mixed with the two AY chips.
// ===========================================================================
static int       ctc_snd_channel       = -1;        // single custom mixer stream
static int       ctc_buffer_len        = 0;         // render frames per video frame
static int       ctc_samplerate        = 0;         // ctc_buffer_len * fps
static int16_t*  ctc_output_buffer     = nullptr;   // rendered at ctc_samplerate
static int16_t*  ctc_stream_buffer     = nullptr;   // resampled (owned by resampler)
static int       ctc_stream_buffer_len = 0;
static double    ctc_phase[2]          = { 0.0, 0.0 };  // phase per tone (ch1, ch2)
static const int CTC_AMPLITUDE         = 8000;          // square-wave amplitude (int16)

// Flagged by the CTC zc1/zc2 callbacks each zero-count; kept for diagnostics
// (the audio itself is gated on z80ctc_getperiod()).
...
static void cchasm_ctc_interrupt(int state)
{
	...
	{ static int n = 0; if ((n++ % 200) == 0) LOG_INFO("CCHASM diag: CTC->Z80 IRQ state=%d (#%d)", state, n); }
...
// CTC ch1/ch2 zero-count callbacks. The tone audio is synthesized in
// cchasm_square_update() from the channel period; here we just flag activity.
static void ctc_timer_1_w(int ch, int data_rising) { (void)ch; if (data_rising) ctc_tone_active[0] = 1; }
static void ctc_timer_2_w(int ch, int data_rising) { (void)ch; if (data_rising) ctc_tone_active[1] = 1; }

// Render both CTC tone channels (zc1/zc2) summed into one buffer. The zc/TO
// output toggles every 'period' seconds, so the square's fundamental is
// 1/(2*period); only the audible band is rendered, which ignores the ultrasonic
// sound-engine IRQ timer and stopped (reset) channels.
static void ctc_audio_render(int16_t* buffer, int length)
{
	double H[2] = { 0.0, 0.0 };   // half-period in samples per tone (0 = silent)
	for (int t = 0; t < 2; t++)
	{
		double period = z80ctc_getperiod(0, t + 1);          // CTC ch1, ch2
		double freq   = (period > 0.0) ? (1.0 / (2.0 * period)) : 0.0;
		if (freq >= 30.0 && freq <= 12000.0)
		{
			H[t] = period * (double)ctc_samplerate;
			if (H[t] < 1.0) H[t] = 1.0;
		}
		else
		{
	...
	for (int s = 0; s < length; s++)
	{
		...
		{
			if (H[t] <= 0.0) continue;
			v += (ctc_phase[t] < H[t]) ? CTC_AMPLITUDE : -CTC_AMPLITUDE;
			ctc_phase[t] += 1.0;
			if (ctc_phase[t] >= 2.0 * H[t]) ctc_phase[t] -= 2.0 * H[t];
		}
...
// Render the CTC tones, resample to the mixer rate, and push the stream
// (phoenix_audio.cpp pattern).
static void cchasm_square_update()
{
	if (!ctc_output_buffer || ctc_snd_channel < 0) return;
	ctc_audio_render(ctc_output_buffer, ctc_buffer_len);
	float ratio = (float)config.samplerate / (float)ctc_samplerate;
	linear_interpolation_16(ctc_output_buffer, ctc_buffer_len,
	                        &ctc_stream_buffer, &ctc_stream_buffer_len, ratio);
	stream_update(ctc_snd_channel, ctc_stream_buffer);
}
...
static int cchasm_io_read_reg(int reg)
{
	...
	case 0x5:
		return input_port_2_r(0);   // buttons (fire/thrust/shield/start)
...
{
	LOG_INFO("CCHASM diag: Z80->CTC ch=%d data=%02x", port & 0x03, data);
...
MEM_ADDR16(0x060000, 0x060001, cchasm_input_port_0_r16, NULL)   // DSW (low byte, MAME AM_READ_PORT)
MEM_ADDR16(0xf80000, 0xf800ff, cchasm_io_r16, NULL)             // soundlatch3/4 + flags + inputs (high byte, MAME 0xff00)
...
int init_cchasm()
{
	...
	ctc_cfg.cpu[0] = CPU1;           // CTC lives on the Z80 sound CPU (per-CPU timer scheduling)
	ctc_cfg.notimer[0] = 0;
	...
	// CTC square-wave tones (fire/thrust): one custom stream, phoenix-style.
	{
		int fps = (Machine->gamedrv->fps > 0) ? Machine->gamedrv->fps : 40;
		ctc_buffer_len = config.samplerate / fps;
		ctc_samplerate = ctc_buffer_len * fps;
		ctc_phase[0] = ctc_phase[1] = 0.0;

		ctc_output_buffer = (int16_t*)malloc((size_t)ctc_buffer_len * sizeof(int16_t));
		if (ctc_output_buffer) memset(ctc_output_buffer, 0, (size_t)ctc_buffer_len * sizeof(int16_t));

		ctc_snd_channel = mixer_alloc_channel(MIXER_CHIP_STREAM_RANGE_LOW, MIXER_FIRST_RESERVED_CHANNEL);
		if (ctc_snd_channel >= 0)
		{
			stream_start(ctc_snd_channel, 0, 16, fps);
			sample_set_volume_mixer(ctc_snd_channel, 110);
		}
	}
...
void run_cchasm()
{
	...
	if (!cchasm_reti_hooked && m_cpu_z80[CPU1] != nullptr)
	{
		...
		m_cpu_z80[CPU1]->reti_hook = []() {
			{ static int n = 0; if ((n++ % 100) == 0) LOG_INFO("CCHASM diag: Z80 RETI (#%d)", n); }
			z80ctc_reti(0);
			};
		...
		m_cpu_z80[CPU1]->int_ack_fn = []() -> int {
			int v = z80ctc_interrupt(0);
			{ static int n = 0; if ((n++ % 100) == 0) LOG_INFO("CCHASM diag: Z80 INT ACK vector=0x%02x (#%d)", v, n); }
			return v;
			};

		cchasm_reti_hooked = true;
	}
	// Synthesize the CTC square-wave tones, then mix the AY-3-8910 chips.
	cchasm_square_update();
	ay8910_sh_update();
...
void end_cchasm()
{
	...
	// Free the CTC custom sound stream (phoenix-style cleanup).
	if (ctc_snd_channel >= 0) { stream_stop(ctc_snd_channel, 0); ctc_snd_channel = -1; }
	if (ctc_output_buffer) { free(ctc_output_buffer); ctc_output_buffer = nullptr; }
	if (ctc_stream_buffer) { delete[] ctc_stream_buffer; ctc_stream_buffer = nullptr; }
	ctc_stream_buffer_len = 0;
	ctc_phase[0] = ctc_phase[1] = 0.0;
	ctc_tone_active[0] = 0;
	ctc_tone_active[1] = 0;
