...
{
	if (bosco_flipscreen != (~data & 1))
	{
		bosco_flipscreen = ~data & 1;
...
READ_HANDLER_NS(bosco_customio_data_2_r)
{
	...
	case 0x91:
		if (offset == 2)
		{
			LOG_INFO("WARNING Returning weird mem location %x", Machine->memory_region[0][0x89cd]);
			return Machine->memory_region[0][0x89cd];
		}
		else if (offset <= 3)
			return 0;
		break;
...
WRITE_HANDLER_NS(bosco_halt_w)
{
	...
	else if (!data)
	{
		cpu_enable(CPU1, 0);
		cpu_enable(CPU2, 0);
		LOG_INFO("CPU enable without reset");
	}
...
int bosco_interrupt_1(void)
{
	...
	if (interrupt_enable_1) { cpu_do_int_imm(CPU0, INT_TYPE_INT); }
}
...
{
	if (interrupt_enable_2) cpu_do_int_imm(CPU1, INT_TYPE_INT);
...
{
	if (interrupt_enable_3) cpu_do_int_imm(CPU2, INT_TYPE_NMI);
