...
static const char *name_b[MAX_JOYSTICK_BUTTONS] = {
 ...
 */
static int internal_joystick_win32_poll()
{
...
static int joystick_win32_init()
{
	...
	win32_joy_num = joyGetNumDevs();
	LOG_INFO("joyGetNumDevs() reports %d joystick(s)", win32_joy_num);
	...
	for (n_joyat = 0; n_joyat < win32_joy_num; n_joyat++) {
		...
		if (joyGetDevCaps(n_joyat, &caps, sizeof(caps)) == JOYERR_NOERROR) {
			// Check if the joystick is physically connected
			js.dwSize = sizeof(js);
			js.dwFlags = JOY_RETURNALL;
			if (joyGetPosEx(n_joyat, &js) == JOYERR_UNPLUGGED) {
				LOG_INFO("Joystick %d is unplugged or not responding", n_joyat);
				continue;
			}

			LOG_INFO("Detected Win32 joystick %d: %s", n_joyat, caps.szPname);
			...
			if (caps.wCaps & JOYCAPS_HASZ) {
				win32_joystick[n_joy].axis_min[n_axis] = caps.wZmin;
				win32_joystick[n_joy].axis_max[n_axis] = caps.wZmax;
				n_axis++;
			}
			if (caps.wCaps & JOYCAPS_HASR) {
				win32_joystick[n_joy].axis_min[n_axis] = caps.wRmin;
			...
			// Register this joystick
			if (win_add_joystick((WINDOWS_JOYSTICK_INFO*)&win32_joystick[n_joy]) != 0) {
				LOG_ERROR("Failed to register joystick %d (%s)", n_joyat, caps.szPname);
				continue;
			}

			LOG_INFO("Joystick %d registered with %d button(s), %d axis/axes", n_joyat,
				win32_joystick[n_joy].num_buttons,
				win32_joystick[n_joy].num_axes);

			n_joy++;
		}
		else {
			LOG_INFO("joyGetDevCaps(%d) failed", n_joyat);
		}
	}
	...
	if (win32_joy_num == 0) {
		LOG_ERROR("No usable Win32 joysticks detected.");
		return -1;
	}

	LOG_INFO("Win32 joystick initialization complete: %d device%s registered",
		win32_joy_num, (win32_joy_num == 1 ? "" : "s"));
...
int install_joystick_win32()
{
	if (_joystick_installed) {
		LOG_INFO("Win32 joystick already installed.");
		return 0;
	}

	LOG_INFO("Installing Win32 joystick driver...");

	clear_joystick_vars();

	if (joystick_win32_init() != 0) {
		LOG_ERROR("joystick_win32_init() failed: no joysticks detected or error occurred.");
		return -1;
	}

	// Directly call internal polling to avoid dependency on installed flag
	if (internal_joystick_win32_poll() != 0) {
		LOG_ERROR("Initial joystick poll failed.");
		return -1;
	}

	_joystick_installed = TRUE;
	LOG_INFO("Win32 joystick installation successful.");
	return 0;
}
 ...
 */
int poll_joystick_win32()
{
	if (_joystick_installed) { internal_joystick_win32_poll(); }
