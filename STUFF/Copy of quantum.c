...
static int deltax=0;
 static   int deltay=0;
  ...
  static int mouse_previous_x=0;
  static int mouse_previous_y=0;
...
static int input_analog[9];
static int input_analog_current_value[9];
static int input_analog_previous_value[9];
static int input_analog_init[9];
static int input_analog_scale[9];
static int input_port_value[9];

/*
void aae_mouse(int *deltax ,int *deltay)
{
   static int mx,my;
		...
		hr = IDirectInputDevice_GetDeviceState(lpdimouse,sizeof(DIMOUSESTATE), &mouse_state );
			*deltax = mouse_state.lX;
			*deltay = mouse_state.lY;
		
		
	 
		 
        write_to_log("%d x %d y",  mouse_state.lX, mouse_state.lY);
...
void update_analog_port(int port)
{
	...
	if (port==0)	delta = mouse_delta_X; 
	if (port==1)    delta = mouse_delta_Y;

	if (port==1)  delta = -delta;
...
static void scale_analog_port(int port)
{
	...
	if (delta >= 0)
		delta = Qscale_by_cycles(delta, 6048000);
	else
		delta = -Qscale_by_cycles(-delta, 6048000);
	...
	input_port_value[port] &= ~0x0f;
	input_port_value[port] |= current & 0x0f;
...
static void read_mouse()
{
   ...
   int now;
   */
   //update_analog_port(0);
  // update_analog_port(1);
   scale_analog_port(0);
   scale_analog_port(1);
  /*
  currentx=turn1;
  ...
  deltay=mickeyy;
    ...
    deltay=-deltay;
   now = Qscale_by_cycles(mouse_current_x - mouse_previous_x, 6048000) + mouse_previous_x;
   
   deltax = now - mouse_last_x;
   mouse_last_x = now;

   now = Qscale_by_cycles(mouse_current_y - mouse_previous_y, 6048000) + mouse_previous_y;
   deltay = now - mouse_last_y;
   mouse_last_y = now;

   write_to_log("DELTAX is %d",deltax);
   write_to_log("DELTAY is %d",deltay);

   if (clip != 0)
	{
		if (deltax*sensitivity/100 < -clip)
			deltax = -clip*100/sensitivity;
		else if (deltax*sensitivity/100 > clip)
			deltax = clip*100/sensitivity;

		if (deltay*sensitivity/100 < -clip)
			deltay = -clip*100/sensitivity;
		else if (deltay*sensitivity/100 > clip)
			deltay = clip*100/sensitivity;
	}
   ...
   turn2=currenty;
	...
	turn1 &= ~0x0f;
	turn1 |= (currentx * sensitivity / 100) & 0x0f;
    turn2 &= ~0x0f;
	turn2 |= (currenty * sensitivity / 100) & 0x0f;

  // turn2+=deltay;
   //turn1 +=deltax;
  //turn1 &= ~0x0f;
 //turn1 |= (currentx * sensitivity / 100) & 0xf;

 // turn2+=deltay;
/*
  if (turn1 < 0) turn1=0x0f;
  else if (turn1 > 0x0f) turn1=0;
  else if (turn2 > 0x0f) turn2=0;
  else if (turn2 < 0) turn2=0x0f;
*/
 
  

   //deltay = (deltay * sensitivity / 100) & 0xf;
  // deltax = (deltax * sensitivity / 100) & 0xf;
   write_to_log("DELTAX is %d",deltax);
   ///if (deltax > 0 ) amt1=deltax;
   //if (deltax < 0 ) amt1=-deltax;
   //if (deltay > 0 ) amt2=deltay;
   //if (deltay < 0 ) amt2=-deltay;

  // turn1+=deltax;
  // turn2+=deltay;

  // if (mickeyx < 0 ) amt1=-mickeyx;
  // if (mickeyy > 0 ) amt2=mickeyy;
  // if (mickeyy < 0 ) amt2=-mickeyy;
...
{ 
 ...
 //  position_mouse(SCREEN_W/2, SCREEN_H/2);     
   ...
   */
	...
	read_mouse();
	 return (input_port_value[0] << 4) | input_port_value[1];
...
void run_quantum()
{
	...
	//write_to_log("FRAME STARTS HERE --------------------------");
		...
		itgone=0;
            ...
            if (mouse_delta_X) update_analog_port(0);
			if (mouse_delta_Y) update_analog_port(1);
...
int init_quantum()
{
    ...
    input_analog_current_value[0]=0;
	input_analog_previous_value[0]=0;
	input_analog_current_value[1]=0;
	input_analog_previous_value[1]=0;
	input_analog_current_value[2]=0;
	input_analog_previous_value[2]=0;
    input_analog_current_value[3]=0;
	input_analog_previous_value[3]=0;
