...
WRITE_HANDLER(led_write)
{
	...
	if ((~data & 0x10)!= lastled1) {lastled1=(~data & 0x10);}
	if ((~data & 0x20)!= lastled2) {lastled2=(~data & 0x20);}
	 
	set_aae_leds(lastled1,lastled2,0);
 }
...
{   
		...
		for (x=0; x < cps; x++)
		{
		dwResult = m6502zpexec( (33600 / cps) ); //cycles
...
int init_spacduel()
{
   ...
   switch(gamenum){                                                                                                              
	...
	if (gamenum==SPACDUEL) BUFFER_SIZE = 44100 / driver[gamenum].fps / 5;
	...
	Pokey_sound_init(1512000, 44100,chip,1 ); // INIT POKEY 1373300 1512000
