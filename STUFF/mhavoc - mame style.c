...
void mhavoc_interrupt( )
{
	...
	//{
		...
		{    write_to_log("IRQ ALPHA CPU");
			...
			m6502zpint(1);
		//	 dwResult =m6502zpexec(300);		// 1500 Instructions for 1.5MHZ CPU
		//		if (0x80000000 != dwResult)
		//			{
		//				m6502zpGetContext(&cpu1);
		//				allegro_message("Invalid instruction at %.2x on CPU 2", cpu1.m6502pc);
		//			}
...
{
	write_to_log(  "Now reading by alpha data sent from gamma: %02x", gamma_data);
	alpha_rcvd=1;
	gamma_xmtd=0;
	return gamma_data;
}
...
{
	write_to_log(  "Now Reading data sent from alpha processor to gamma: %02x", alpha_data);
	gamma_rcvd=1;
	alpha_xmtd=0;
	return alpha_data;
}
...
WRITE_HANDLER( mhavoc_gamma_w)
{
	...
	//nmicpu1=1;
	//noint=1;
	write_to_log("--------------------------RELEASING CPU SLICE TO GAMMA, cpunum = %d",cpuhere);
   /*
	  m6502zpGetContext(&cpu1);	// Get CPU #1's state info
      m6502zpSetContext(&cpu2);	// Set CPU #2's information
	  m6502zpnmi();
	 // do{
	  m6502zpexec(35);cycles+=20;
	//  } while (gamma_xmtd==0 && cycles < 1000);
      write_to_log("--------------------------Cycles for transmit %d",cycles);
...
{   
	...
	dwElapsedTicks = m6502zpGetElapsedTicks(0);
		...
		for (x=0; x < 36; x++)
		{
			    ...
			    cpuhere=1;
				//write_to_log("Starting run, cpu1");
				m6502zpSetContext(&cpu1);	// Set CPU #1's information
				dwResult = m6502zpexec(5000/2);		// 1500 Instructions for 1.5MHZ CPU
				if (0x80000000 != dwResult)
					{
						m6502zpGetContext(&cpu1);
						allegro_message("Invalid instruction at %.2x on CPU 0", cpu1.m6502pc);
					}
				//if (intcpu1) {m6502zpint(1);intcpu1=0;}
				m6502zpGetContext(&cpu1);	// Get CPU #1's state info
				...
				if (teller==0){
                 cpuhere=2;
				//write_to_log("Starting run, cpu2");
                m6502zpSetContext(&cpu2);	// Set CPU #1's information
			    if (reset1) {m6502zpreset(); reset1=0;write_to_log("Reset, Gamma CPU");}
				dwResult =m6502zpexec(2300/2);		// 1500 Instructions for 1.5MHZ CPU
				if (0x80000000 != dwResult)
					{
						m6502zpGetContext(&cpu2);
						allegro_message("Invalid instruction at %.2x on CPU 2", cpu2.m6502pc);
					}
				if (intcpu2) {m6502zpint(1);intcpu2=0;}
				m6502zpGetContext(&cpu2);	// Get CPU #1's state info
				}
				else {
	  ...
	  m6502zpnmi();
	  do{
	  m6502zpexec(20);cycles++;
	  } while (gamma_xmtd == 0 && cycles < 10);
      write_to_log("--------------------------Cycles for transmit %d",cycles);cycles=0;
	  teller=0;
	  m6502zpGetContext(&cpu2);	// Get CPU #1's state info
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////
				/*m6502zpSetContext(&cpu2);	// Set CPU #1's information
			    if (reset1) {m6502zpreset(); reset1=0;write_to_log("Reset, Gamma CPU");}
				dwResult =m6502zpexec(700);		// 1500 Instructions for 1.5MHZ CPU
				if (0x80000000 != dwResult)
					{
						m6502zpGetContext(&cpu2);
						allegro_message("Invalid instruction at %.2x on CPU 2", cpu2.m6502pc);
