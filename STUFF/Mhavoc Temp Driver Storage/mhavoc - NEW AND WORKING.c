...
void mhavoc_interrupt( )
{
	...
	{
		alpha_irq_clock++;
		if ((alpha_irq_clock & (0x30)) == (0x30)) //0x0c
		{   // write_to_log("IRQ ALPHA CPU");
		     set_pending_interrupt(INT_TYPE_INT, 0);
		     //intcpu1=1;
	...
	if (has_gamma_cpu)
	{
		...
		if (gamma_irq_clock & (0x20)) //08
		{
		 // write_to_log("IRQ GAMMA CPU");
		  set_pending_interrupt(INT_TYPE_INT, 1);
		  //intcpu2=1;
		  //m6502zpSetContext(c6502[1]);	
         //m6502zpint(1);
		  //m6502zpGetContext(c6502[1]);
...
WRITE_HANDLER(avg_go)
{
   //if (MHAVGDONE==0) {return;}
	...
	if (total_length > 1) {MHAVGDONE=0;get_video_ticks(0xff);//mhticks=m6502zpGetElapsedTicks(0xff);
...
READ_HANDLER(mhavoc_port_0_r)
{
	...
	// Emulate the 2.4Khz source on bit 2 (divide 2.5Mhz by 1024)  (EVERY 120 CYCLES)
	  if (get_eterna_ticks(0) & 0x400) //0x400
	  //if (bitflip)
	  {res &=~0x02;}
	  else
	  { res|=0x02;}
...
//----------------------------------------------------------------------------------------------------------------------   		
		        ...
		        if (nmicpu1)  { m6502zpnmi(); nmicpu1=0;}
