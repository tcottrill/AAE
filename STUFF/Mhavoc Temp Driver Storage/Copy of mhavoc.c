...
void mhavoc_interrupt()
{
	...
	//{
		gamma_irq_clock++;
		if (gamma_irq_clock & (0x20)) //08
		{
		  write_to_log("IRQ GAMMA CPU, Gamma clock %x",gamma_irq_clock);
		  intcpu2=1;
		  gamma_irq_clock = 0;
...
WRITE_HANDLER(avg_go)
{
   //if (MHAVGDONE==0) {return;}
	...
	if (total_length > 1) {MHAVGDONE=0;mhticks=get_video_ticks(0xff);//
...
READ_HANDLER(mhavoc_port_0_r)
{
	...
	ticks = get_video_ticks(0);//m6502zpGetElapsedTicks(0);
	//if (MHAVGDONE==0 )write_to_log("Total LENGTH HERE %f and TOTAL TICKS %d",me,ticks );
	if ( get_video_ticks(0) > me && MHAVGDONE==0 ) {MHAVGDONE=1;total_length=0;}
...
READ_HANDLER(input_port_2_r)
{
  ...
  // get_mouse_mickeys(&mickeyx, &mickeyy);
   ...
   if (mickeyx > 0 ) amt1-=1+(sensitivity *( mickeyx * mickeyx / 80)); //20
   if (mickeyx < 0 ) amt1=1+(sensitivity * ( mickeyx * mickeyx / 80));
...
READ_HANDLER(alphaone_port_0_r)
{
	...
	me = (((1780 * total_length)/ 1000000) * 1512); //1780
	...
	if (KeyCheck(config.kreset))  {run_reset();}
		...
		for (x=0; x < (passes+1); x++) 
		{
				...
				cpu_now=0;
				 ...
				 write_to_log("Ticks here on CPU #1 %d",m6502zpGetElapsedTicks(0));
...
//----------------------------------------------------------------------------------------------------------------------   		
				...
				if (0x80000000 != dwResult)
			     ...
			     write_to_log("Ticks here on CPU #2 %d",m6502zpGetElapsedTicks(0));
...
int init_mhavoc(void)
{
	...
	if (has_gamma_cpu){
					   ...
					   init6502B(GammaRead,GammaWrite,1);
	                   memset(gameImage[1]+0x6000, 0xff, 0x1ff);
					   
	                  }
	else {
		   init6502B(AlphaOneRead,AlphaOneWrite,0);
