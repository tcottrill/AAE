...
void check_spinner()
{
 int temp=0;
  ...
  update_mouse(&mickeyx ,&mickeyy);
   //turnh is the current location of the spinner
   amt1=0;
   if (config.mouse1x_invert){mickeyx=-mickeyx;}
   mickeyx=-mickeyx;
   if (mickeyx > 0 ) 
   {
	   amt1=mickeyx;//1+(sensitivity *( mickeyx * (mickeyx / 256)));
	  // if (amt1 > 45) amt1=45;
	  
	  
	   //if ( (turnh+amt1)> 256) amt1=(turnh+amt1)-256;
   }
   if (mickeyx < 0 ) 
   {
        amt1=mickeyx;////amt1=1+(sensitivity * ( mickeyx * (mickeyx / 256)));
	   //amt1=-amt1;
	  // if (amt1 < -45)  amt1=-45;

	   //if ( (turnh+amt1)< 0) amt1=256 +(turnh+amt1);
	
   
   }
   
   if (amt1 > 0xff || amt1 < -0xff) amt1=0;
 
   if (key[config.kp1right]) turnh=turnh-7;
   if (key[config.kp1left])  turnh=turnh+7;
  
   turnh+=amt1;
   //glPrint(200, 580, 255,255,255,255,1,0,0,"VAL %d",amt1);
   //write_to_log("SPINNER VAL %d",amt1);
 // return turnh;
}
...
player_1=0;
cache_clear();
m6502zpSetContext(&cpu1);	
m6502zpreset();
m6502zpGetElapsedTicks(0xff);
...
void mhavoc_interrupt( )
{
	...
	{
		alpha_irq_clock++;
		if ((alpha_irq_clock & (0xc)) == (0x0c)) //0x0c
...
WRITE_HANDLER( dual_pokey_w )
{
	...
	if  (pokey_reg[pokey_num]==0x0f) {pkenable[pokey_num]=data;}
    switch (pokey_num) {
        case 0:	Update_pokey_sound( pokey_reg[0],data,0,6); break;
        case 1:	Update_pokey_sound( pokey_reg[1],data,1,6); break;
       }
}
...
{ write_to_log("---------------------------AVGDVG RESET ------------------------");total_length=0;}
...
WRITE_HANDLER(avg_go)
{
   //if (MHAVGDONE==0) {return;}
	...
	if (total_length > 1) {MHAVGDONE=0;mhticks=m6502zpGetElapsedTicks2(0xff);
	                       //write_to_log("Total LENGTH HERE %d ",total_length );
	                      } else {MHAVGDONE=1;}
	//}
...
READ_HANDLER(mhavoc_port_0_r)
{
	...
	me = (((1780 * total_length)/ 1000000) * 1512);//* 1512); 
	ticks=m6502zpGetElapsedTicks2(0);
	...
	if (player_1==0)
	{
	if (key[config.ktestadv])  bitclr(res,0x20); //PORT_BITX( 0x20, IP_ACTIVE_LOW, IPT_SERVICE, "Diag Step/Coin C", KEYCODE_F1, IP_JOY_NONE )
	if (key[config.kcoin1])    bitclr(res,0x40);//PORT_BIT ( 0x40, IP_ACTIVE_LOW, IPT_COIN1 )	/* Left Coin Switch  */
	if (key[config.kcoin2])    bitclr(res,0x80);//PORT_BIT ( 0x80, IP_ACTIVE_LOW, IPT_COIN2 )	/* Right Coin */
	}
	...
	if (player_1)
	{
	  ...
	  if (config.hack)             bitclr(res,0x40);
	}
   	return (res & 0xff);
}
...
READ_HANDLER(mhavoc_port_1_r)
{
	...
	if (key[config.kp1but1]  || (mouse_b & 1))  bitclr(res,0x80);
    if (key[config.kp1but2]  || (mouse_b & 2))  bitclr(res,0x40);
...
READ_HANDLER(input_port_2_r)
{
  ...
  // get_mouse_mickeys(&mickeyx, &mickeyy);
   ...
   if (mickeyx > 0 ) 
   {
	   amt1-=(mickeyx);//1+(sensitivity *( mickeyx * mickeyx / 80));
	   if (amt1 <- 0x2f) amt1=-0x2f;
	   if ( (turnh+amt1)< 0xff) amt1=0xff +(turnh+amt1);
   }
   if (mickeyx < 0 ) 
   {
       amt1=(mickeyx);//1+(sensitivity * ( mickeyx * mickeyx / 80));
	   if (amt1 > 0x2f)  amt1=0x2f;
	   if ( (turnh+amt1)> 0xff) amt1=(turnh+amt1)-0xff;
   
   }
   //glPrint(200, 580, 255,255,255,255,1,0,0,"Sensitivity %d",amt1);
  ...
  // if (mickeyx > 0  || mickeyx < 0){
  // if ( (turnh+amt1)> 0xff) temp=(turnh+amt1)-0xff;
  // if ( (turnh+amt1)< 0xff) temp=0xff +(turnh+amt1);
  // }
   turnh+=amt1;
   
   //if (turnh<0) turnh=0;
  //if (turnh>0xff) turnh=0xff;
  */
  return turnh;
}
...
DRAWCODE:	if (z == 2){z = statz;}if (z) {z = (z << 4) | 0x1f;}
			...
			total_length+=vector_timer(myx, myy); 
			 ...
			 if (z>0)
			 {      
				    if (sparkle) { color =0xf +   (((spkl_shift & 1) << 3) | (spkl_shift & 4)
                    ...
                    if (sparkle)
					{
						...
						while ( ex != sx || ey != sy )
						{
						 ...
						 escape ++;
                         //if (escape > 40) write_to_log("SX  %f  EX %f SY %f EY %f",sx,ex,ey,sy);
                         add_color_point(sx, sy,red,green,blue);
			...
			case JSRL:
				...
				{   stack [sp] = pc;
					if (sp == (MAXSTACK - 1))
					{
						write_to_log ("--- Passed MAX STACK (BAD!!) ---");
						done = 1;
						sp = 0;
					}
					else
...
{
 //if (data !=0x50 && data !=0) write_to_log("WatchDog Barking!!!!!!!!!!!!!!!! ------x data:%x",data);
}

READ_HANDLER(eprom_r)
{
	//write_to_log("EEPROM READ %x",address);
	return gameImage[1][address];
}

WRITE_HANDLER(eprom_w)
{
	gameImage[1][address] = data;
}
...
{   
	int x;
    int bugger=0;
   	int cycles1 = (2500000 /50);
	int cycles2 = (1250000 /50);
	int passes=400; 
	...
	m6502zpGetContext(&cpu1);
    m6502zpSetContext(&cpu2);
	//write_to_log("LAST TOTAL CYCLES RAN CPU2 FRAME %d",m6502zpGetElapsedTicks(0));
    dwElapsedTicks = m6502zpGetElapsedTicks(0xff);
	m6502zpGetContext(&cpu2);
...
//----------------------------------------------------------------------------------------------------------------------   		
                ...
                // write_to_log("In CPU 2");
			    if (reset1) { m6502zpreset(); 
				              reset1=0;
							  write_to_log("Reset, Gamma CPU");
				              pokey_cpu_reset();  
				            }
		        if (nmicpu1)  {   
					                nmicounter++;
					               ...
					               //write_to_log("NMI Taken, Gamma CPU");
								 }		
				dwResult =m6502zpexec( (cycles2 / passes));		
				if (0x80000000 != dwResult)
					{
...
int init_mhavoc(void)
{
	...
	set_mouse_range(0, 0, config.screenw, config.screenh); 
	...
	if (has_gamma_cpu){
	                   ...
	                   memset(gameImage[1]+0x6000, 0xff, 0x1ff);
					   mhavoc_sh_start();
					   //Pokey_sound_init(1250000, 44100,4,6 ); 
	                  }
	else {
		   mhinit6502(AlphaOneRead,AlphaOneWrite,0);
		   //Pokey_sound_init(1250000, 44100,2,6 );
		   memset(gameImage[0]+0x1800, 0xff, 0xff);
	     }
