...
static int scale_by_cycles(int val, int clock)
{
	int k;
	int current=0;
	int max;
...
static void mh_pokey_update()
{
  int newpos=0;
   ...
   newpos = scale_by_cycles(BUFFER_SIZE, 1250000);	
...
WRITE_HANDLER(avg_go)
{
   //if (MHAVGDONE==0) {return;}
	...
	if (total_length > 1) {MHAVGDONE=0;get_vidticks(0xff);//mhticks=m6502zpGetElapsedTicks(0xff);
...
//----------------------------------------------------------------------------------------------------------------------   		
		...
		write_to_log("cycles ran this frame %d - wanted to run %d",ticktest,cycles2); 
}
