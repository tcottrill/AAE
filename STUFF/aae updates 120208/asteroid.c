...
{  
    ...
    return rand() & 0xff;//pokey_random[chip];
}
...
{   
	...
	dvggo=1;cache_clear();total_length=0;dvg_generate_vector_list();set=1;vec_done=1;//write_to_log("DVG GO CALLED THIS FRAME");
	if (total_length==0)  {dvggo=0;vec_done=0;set=0;aticks2=get_video_ticks(0xff);
	 }//write_to_log("DVG GO SCRATCHED ---------- THIS FRAME");}
...
void run_asteroids()
{
	...
	if (KeyCheck(KEY_F2))      {testsw^=1; cpu_needs_reset(0);}
