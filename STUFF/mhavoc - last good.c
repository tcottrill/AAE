...
READ_HANDLER(mh_quad_pokey_read)
{
	...
	if (pokey_reg[pokey_num]==0x08) return 0;//z80dip1; //Dipswitch Read
...
READ_HANDLER(mhavoc_port_0_r)
{
	...
	me = (((1780 * total_length)/ 1000000) * 1500);//* 1500); //2500 //1775
...
DRAWCODE:	if (z == 2){z = statz;}if (z) {z = (z << 4) | 0x1f;}
			...
			total_length+=vector_timer(myx, myy); 
			 ...
			 {      
					...
					clip=330; //330 //512 on title screen
					...
					//if (ey==ex && sy==ey){add_color_point(sx, sy,red,green,blue);}
...
{
 write_to_log("WatchDog Barking!!!!!!!!!!!!!!!! ------x data:%x",data);
}
...
WRITE_HANDLER(speech_w)
{
	//write_to_log("Ram write from Gamma CPU address:%x data:%x",address,data);
	//gameImage[1][address] = data;
}
WRITE_HANDLER(speech_strobe)
{
	//write_to_log("Ram write from Gamma CPU address:%x data:%x",address,data);
	//gameImage[1][address] = data;
}
...
{   
	...
	write_to_log("---------------------------AVGDVG RESET ------------------------");
		...
		{
			   m6502zpSetContext(&cpu1);
			   dwResult = m6502zpexec(cycles / passes);		// 1500 Instructions for 1.5MHZ CPU
				if (0x80000000 != dwResult)
					{
						m6502GetContext(&cpu1);done=1;
	...
	MHAVGDONE=1;
		...
		for (x=0; x < 8; x++)
		{
				...
				dwResult = m6502zpexec(cycles/8);		// 1500 Instructions for 1.5MHZ CPU
				if (0x80000000 != dwResult)
					{
						m6502zpGetContext(&cpu1);
						allegro_message("Invalid instruction at %.2x on CPU 0", cpu1.m6502pc);
...
{   
	...
	int cycles2 = (1250000 /50);//1250000
	int addon=0;
    int testcyc=0;
	...
	write_to_log("------------FRAME START --------------"); 	
		for (x=0; x < passes; x++) //160
		{
				...
				// if (intcpu1) {m6502zpint(1);intcpu1=0;write_to_log("INT Taken, Alpha CPU");} 
				dwResult = m6502zpexec((int) cycles1 / passes);		// 1500 Instructions for 1.5MHZ CPU
				//if (addon) {dwResult = m6502zpexec(cycles);addon=0;}
				if (0x80000000 != dwResult)
					{
						//m6502zpGetContext(&cpu1);
				...
				if (teller){ //I have released my timeslice.
					addon=1;
					cycles =  (m6502zpGetElapsedTicks(0)-testcyc);	//((cycles1 / passes) *x) -
					cycles -=20;
				//if (cycles > 0) write_to_log("Cycles start at release %d ", cycles);	
                if (cycles < 1) addon=0;
			   // m6502zpexec(20);//Run a few?    250us worth of cycles. //62
                teller=0;
				}
 ...
 //----------------------------------------------------------------------------------------------------------------------   		
			    ...
			    if (reset1) {m6502zpreset(); reset1=0;write_to_log("Reset, Gamma CPU");}
				 if (nmicpu1)  {    nmicounter++;
					              
								   if (nmicounter==3) { m6502zpnmi(); 
								                        nmicpu1=0;
														nmicounter=0;
								                        gamma_rcvd=0;
														alpha_xmtd=1;
		...
		bugger^=1; if (bugger) mhavoc_interrupt();
