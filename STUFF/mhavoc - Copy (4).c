...
READ_HANDLER(mhavoc_port_0_r)
{
	...
	me = (((1300 * total_length)/ 1000000) * 1500);//* 1500); //2500 //1775
...
{   
	...
	int cycles2 = (1250000 /30);//1250000
	int addon=0;
    int testcyc=0;
	...
	dwElapsedTicks = m6502zpGetElapsedTicks(0xff);
		...
		for (x=0; x < passes; x++) //160
		{
                ...
                if (cycles < 1) addon=0;
			    m6502zpexec(22);//Run 250us worth of cyces.
                teller=0;
				}
...
int init_mhavoc(void)
{
   ...
   mhinit6502(GammaRead,GammaWrite,1);
	...
	BUFFER_SIZE = 44100 /50 / 2;// driver[gamenum].fps
