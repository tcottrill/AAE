...
WRITE_HANDLER(avg_go)
{
	...
	if (total_length > 1300) {drawnok=1;MHAVGDONE=0;mhticks=m6502zpGetElapsedTicks(0xff);} else {MHAVGDONE=1;drawnok=0;}
	}
...
READ_HANDLER(mhavoc_port_0_r)
{
	...
	me = (((1775 * total_length)/ 1000000)* 200); //2500
...
DRAWCODE:	if (z == 2){z = statz;}if (z) {z = (z << 4) | 0x1f;}
			...
			if (sparkle) {color =  randintm(0x1f);}
			// if (z>0)
			// {      
				 ...
				 if (vec_colors[color].b) blue=z; else blue=0;
					...
					if (draw) {     add_color_line(sx, sy,ex,ey,red,green,blue);
...
{   
    ...
    drawnok=0;
		...
		for (x=0; x < 160; x++) //160
		{
				...
				dwResult = m6502zpexec((int) cycles1 / 125);		// 1500 Instructions for 1.5MHZ CPU
				if (addon) {dwResult = m6502zpexec(332);addon=0;}
 ...
 //----------------------------------------------------------------------------------------------------------------------   		
				...
				dwResult =m6502zpexec((int) cycles2 / 125 );		// 1500 Instructions for 1.5MHZ CPU
				if (0x80000000 != dwResult)
					{
		...
		if (x==37)  do_sound();//mhavoc_sh_update();		
		if (x==72)  do_sound();	
		if (x==110)  do_sound();	
		}
		do_sound();	
...
int init_mhavoc(void)
{
   ...
   mhinit6502(GammaRead,GammaWrite,1);
	...
	BUFFER_SIZE = 44100 / driver[gamenum].fps / 4;
	BUFFER_SIZE +=4;
