...
UINT8 cpu_z80::mz80GetMemory(uint16_t addr)
{
	...
	while (MemRead->lowAddr != 0xffffffff)
	{
		...
		{
			z80pc = GetPC();
			return (MemRead->memoryCall(addr, MemRead)); //return (MemRead->memoryCall(addr - MemRead->lowAddr, MemRead));
			 //
		}
...
void cpu_z80::mz80PutMemory(uint16_t addr, uint8_t byte)
{
	...
	while (MemWrite->lowAddr != 0xffffffff)
	{
		...
		{
			z80pc = GetPC();
			MemWrite->memoryCall(addr, byte, MemWrite); //MemWrite->memoryCall(addr - MemWrite->lowAddr, byte, MemWrite);
			return;
		}
		MemWrite++;
	}
...
unsigned long int cpu_z80::mz80exec(unsigned long int cCyclesArg)
{
	...
	while (cCycles > 0)
	{
		...
		z80pc = GetPC(); /* for some of the platforms */
		//wrlog("Opcode here is %x pc is %x ", bOpcode, z80pc );
