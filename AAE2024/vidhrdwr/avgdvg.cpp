...
static void run_state_machine(int dummy)
{
	...
	while (cycles < 600000) //cycles < VGSLICE)// && cycles < 440000)//) 
	{
		/* Get next state */
		vg->state_latch = (vg->state_latch & 0x10) | (vg->state_prom[vgc->state_addr(vg)] & 0xf);
		//wrlog("Prom data %x   State Latch %x", vg->state_prom[vgc->state_addr(vg)] & 0xf, vg->state_latch &7);
		if (ST3)
		{
			/* Read vector RAM/ROM */
		...
		if (vg->halt && !(vg->state_latch & 0x10))
		{
			...
			timer_pulse((TIME_IN_HZ(MASTER_CLOCK) * cycles), CPU0, vg_halt_setter);
			//wrlog("End AVG Update, timer set to %f , cycles %d", (TIME_IN_HZ(MASTER_CLOCK) * cycles), cycles);
			cycles = 2000000;
		}
...
{
	//wrlog("Starting AVG Update");
	if (vector_updates) return;
