...
static int do_key_pressed(int keycode)
{
	int k;

	k = key[keycode];

	if (k == KEY_PAUSE || k == KEY_ENTER) k = 0;
	return k;
}
...
static void do_trak_read(int* medeltax, int* medeltay)
{
	//aae_mouse(medeltax,medeltay);
	;
}
...
static void check_joystick(void)
{
	...
	for (x = 0; x < num_joysticks; x++) {
		for (y = 0; y < 4; y++) {
			if (joy[x].stick[y].axis[0].d1) { my_j[(x * 44) + (4 * y) + 1] = 1; }//wrlog("hit on my joy %d",((x*44)+(4*y)));}
			if (joy[x].stick[y].axis[0].d2) { my_j[(x * 44) + ((4 * y) + 2)] = 1; }//wrlog("hit on my joy %d",((x*44)+(4*y)));}
			if (joy[x].stick[y].axis[1].d1) { my_j[(x * 44) + ((4 * y) + 3)] = 1; }//wrlog("hit on my joy %d",((x*44)+(4*y)));}
			if (joy[x].stick[y].axis[1].d2) { my_j[(x * 44) + ((4 * y) + 4)] = 1; }//wrlog("hit on my joy %d",((x*44)+(4*y)));}
	  // if (joy[x].stick[y].flags & JOYFLAG_ANALOGUE)	{wrlog("analog stick %d axis 0 %d",y,joy[x].stick[y].axis[0].pos);wrlog("analog stick %d axis 1 %d",y,joy[x].stick[y].axis[1].pos);}
		}
...
static int is_pressed(int i)
{
	...
	//if (x==15)
	//	wrlog("Port here: %d Key %d",x,GK[i].Current );
...
static void set_port(int i)
{
	...
	if ((GK[i].Type & ~INF_MASK) == IN_ACTIVE_LOW)      PORT[x] &= ~GK[i].Mask;

	else
		if ((GK[i].Type & ~INF_MASK) == IN_ACTIVE_HIGH) PORT[x] |= GK[i].Mask;
}

void Check_Input(int in, int state)
{
	//wrlog("Checking input %d",inkey);  set_port(int i)
	int p = GK[in].Port;
	int val = p + in;
...
void proc_input(int num)
{
	...
	{
		for (i = 0; i < MAX_INPUT_PORT; i++) { PORT[i] = 0; }
	}
	i = 0; //Reset i so I can use it again.

	//wrlog("START INPUT PROCESSING-----");

	while (GK[i].Port > -1)
	{
		//wrlog("GK PORT %d",GK[i].Port);
		//wrlog("GK ID %x",GK[i].Type & 0xff);
		//wrlog("here! %s ",GK[i].Heading );
		...
		if (toggle_keys[num][i]) set_port(i);
		...
		if (is_pressed(i))
		{
			...
			else if (GK[i].Type & INF_RESETCPU)  //Should CPU 0 be reset for this?
			{
				if (debounce[i] == 0)
				{
					...
					cpu_needs_reset(0);
					debounce[num][i] = 1;
					//wrlog("Reset on CPU 0 Called by F3");
					set_port(i);
				}
			}
			...
			/*
		   else if ((GK[i].Type & ~INF_MASK) == IN_ACTIVE_HIGH  ) //Actual Input?
			{
				   PORT[GK[i].Port] |= GK[i].Mask;
					   //allegro_message("HEY %d",PORT[GK[i].Port]);
			}

			else if  ((GK[i].Type & ~INF_MASK) == IN_ACTIVE_LOW ) //Actual Input?
			{
				  PORT[GK[i].Port] = PORT[GK[i].Port] & ~GK[i].Mask;
			}
		   */
			waspressed[num][i] = 1;
		}
...
static void update_analog_port(int port) //actually input number not port really.
{
	...
	// wrlog("UPDATING analogport Number %s %d ",GK[port].Heading,port);
	...
	default:
		...
		wrlog("Oops, polling non analog device in update_analog_port()????: TYPE %d", type);
	}
	...
	if (GK[port].Type & INF_PORTRESET) current = 0;
	else current = input_analog_current_value[p];
	...
	if (axis == X_AXIS) { delta = mouse_delta_X; }//[player];
	else { delta = mouse_delta_Y; }//[player];

	//If the user is using Keys, Override the delta with the key delta
	if (PORT[GK[port - 1].Port]) delta -= keydelta;
	if (PORT[GK[port + 1].Port]) delta += keydelta;
	...
	if (is_stick)
	{
		...
		//wrlog("I think this is an analog stick?");
		/* center stick */
		if ((delta == 0) && (GK[port].Type & INF_CENTER))
		{
			// wrlog("CENTERED STICK!!!");
	...
	current += delta;
	//  wrlog("SCALED ANALOG DELTA is %d",current);
	if (check_bounds)
	{
		//wrlog("I am checking bounds for some reason");
	...
	// wrlog("delta this port %d",delta);
	input_analog_current_value[p] = current;
	//wrlog("Current value NOW for this port  %d",input_analog_current_value[p]);
}
...
static void scale_analog_port(int port)
{
	...
	sensitivity = GK[port].Mask;
	// wrlog("SCALING analogport Number %d with sensitivity %d",p, sensitivity);
	delta = input_analog_current_value[p] - input_analog_previous_value[p];
	//wrlog("delta pre scaling is %d",delta);
	...
	if (driver[gamenum].cpu_freq[0] == 2500000)
	{
		...
		if (delta >= 0)
			...
			delta = -MHscale_by_cycles(-delta, 2500000);
		//wrlog("USING MH SCALE TO CYCLES!!!! DELTA = %d",delta);
		*/
	}
	else
	{
	...
	//wrlog("delta POST scaling is %d",delta);
...
int getport(int port)
{
	...
	//wrlog("Game Get Port Called, Port %d Data %d",p,in);
	return PORT[port];
}
...
void update_input()
{
	...
	check_joystick(); //Update digital joystick settings.
	 ...
	 //Update Default Keys, for menu and etc.
	 //GK =  default_keys; //driver[0].game_keys; //
	switch_keys(-1);
	//test_keys();
	c = 0;
	while (GK[c].Port > -1) {
		//wrlog("ROUND 3 -----Key Number %d : Setting Key %x Setting Joy %x",c,GK[c].Current,GK[c].Joystick);
		c++;
	}
	...
	switch_keys(gamenum);
	proc_input(1);
	//Update Game specific keys
   // GK = driver[gamenum].game_keys;
	c = 0;
	while (GK[c].Port > -1) {
		//wrlog("ROUND 4 -----Key Number %d : Setting Key %x Setting Joy %x",c,GK[c].Current,GK[c].Joystick);
		c++;
	}
	...
	while (GK[i].Port > -1)
	{
		...
		if ((GK[i].Type & 0xff) == IN_ANALOG)
		{
			...
			/*
						   wrlog("PORT NUMBER %d",GK[i].Port);
						   wrlog("PORT NUMBER %d",GK[i].Mask);
						   wrlog("PORT HEADING %s",GK[i].Heading);
						   wrlog("PORT NUMBER %d",GK[i].Default);
						   wrlog("PORT NUMBER %d",GK[i].Current);
						   wrlog("PORT NUMBER %d",GK[i].Joystick);
						   wrlog("PORT NUMBER %d",GK[i].Type);
						   wrlog("PORT NUMBER %d",GK[i].Arg1);
						   wrlog("PORT NUMBER %d",GK[i].Arg2);
	...
	while (GK[i].Port > -1 && (i < MAX_KEY))
	{
		if (GK[i].Type != IN_DEFAULT)
		{
			if ((GK[i].Type & ~INF_MASK) != IN_ANALOG)
			{
				...
				//walk_keyset(i);
				//wrlog("Setting Joy %x",GK[i].Joystick);
				wrlog("SETTING DEFAULT KEY Number %d : Setting Key %x Setting Joy %x", i, GK[i].Current, GK[i].Joystick);
			}
		}
		i++;
	}
...
void reverse_force_keys()

{
	int i = 0;

	wrlog("REVERSE FORCE KEYS CALLED %d", gamenum);
}

void force_keys()
{
	int i = 0;
	// wrlog("OK STARTING TO FORCE SET KEYS");
	while (GK[i].Port > -1 && (i < MAX_KEY))
	{
		if ((GK[i].Type & ~INF_MASK) != IN_DEFAULT)
		{
			if ((GK[i].Type & ~INF_MASK) != IN_ANALOG)
			{
				...
				GK[i].mouseb = GK[i].Default >> 24;
				//walk_keyset(i);
				wrlog("Key Number %d : Setting Current %x Setting Joy %x", i, GK[i].Current, GK[i].Joystick);
			}
		}
...
int retrieve_keys()
{
	...
	strcpy(fullpath, "keycfg\\");
	if (gamenum == 0)strcat(fullpath, "default");
	else strcat(fullpath, driver[gamenum].name);
	strcat(fullpath, ".aae");

	fp = fopen(fullpath, "r");

	if (!fp) { wrlog("Saved key configuration not found, Setting Defaults"); if (gamenum > 0) force_keys(); else set_default_keys(); return 0; }
	else {
		wrlog("Loading saved key configuration");
		while (fscanf(fp, "%d ", &d) != EOF)
		{
			GK[i].Current = d;
			wrlog("Key loaded %x", d);
			//walk_keyset(i);
			wrlog("val loaded %d", d);
			i++;
		}
	}
	fclose(fp);
...
int save_keys()
{
	...
	if (!fp) { wrlog("Could not Save KeySet!"); return 0; }
	else {
		do {
			d = GK[i].Current;
			fprintf(fp, "%d ", d);
...
{
	if (num > -1) GK = &driver[gamenum].game_keys[0];
	else GK = FOO;//&default_keys[0];
...
void menu_test(int num)
{
	int i = 0;
	if (num > -1) MK = &driver[gamenum].game_keys[0];
	else MK = FOO;//&default_keys[0];

	//GAMEKEYS *MK = &default_keys[0];

	while (MK[i].Port > -1) {
		wrlog("KEY PORT %x", MK[i].Port);
		wrlog("KEY MASK %x", MK[i].Mask);
		wrlog("KEY HEADING %s", MK[i].Heading);
		wrlog("Default %x", MK[i].Default);
		wrlog("Current %x", MK[i].Current);
		//wrlog("REAL CURRENT %x",default_keys[i].Current);
		i++;
	}
}
/*
...
{
   int i=0;
   static int hasrun2=0;
   static int menukeys[256];

  if (hasrun2==0) //INIT
   {
	  memset(menukeys,0,256);
	  hasrun2=1;
   }
...
int GkeyCheck(int keynum)
{
  ...
  if (hasrun2==0) //INIT
   ...
   if (!gkeys[keynum]&&key[keynum]) //Return True if not in que
   {
	  gkeys[keynum]=1;
	  return 1;
   }
   else if (gkeys[keynum]&&!key[keynum]) //Return False if in que
	  gkeys[keynum]=0;
   return 0;
}
