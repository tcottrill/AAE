...
void AVG_RUN(void)
{
	...
	// {;}//wrlog("VGO with zeroed vector memory at %x\n",PC);return;}//wrlog("VGO with zeroed vector memory at %x\n",PC); }
	//wrlog("STARTING VG DRAW");
	pc = PCTOP;
		...
		DRAWCODE:			if (z == 2) { z = statz; }
			...
			if (z > -1)
			{
				if (TYPE_BZ) BZ_DRAW(currentx, currenty, deltax, deltay, z, color);
					...
					//wrlog("red %f green %f blue %f z %d",vec_colors[color].r,vec_colors[color].g,vec_colors[color].b,z);
					// glColor4ub(vec_colors[color].r,vec_colors[color].g,vec_colors[color].b,z);
					 //if (vec_colors[color].r) red=z; else red=0;
					// if (vec_colors[color].g) green=z; else green=0;
					 //if (vec_colors[color].b) blue=z; else blue=0;
					 //Crude Color ra,ga,ba;
...
void avg_init()
{
	...
	if (gamenum == BZONE ||
		...
		gamenum == GRAVITR2 ||
		gamenum == GRAVP) {
		TYPE_GV = 1; PCTOP = 0x2000; set_sd_colors(); NO_CACHE = 1;
	}
