...
WRITE_HANDLER(mhavoc_rom_banksel_w)
{
	...
	//wrlog("Alpha ROM Bank select: %02x",data);
}
...
WRITE_HANDLER(MWA_BANK1_W)
{
	...
	GI[CPU0][bank] = data;
	//wrlog("Alpha banked ram Write: address %x data %x",bank,data);
}
...
READ_HANDLER(MRA_BANK1_R)
{
	...
	bank = bank + (address - 0x200);
	//wrlog("Alpha Banked ram Read: address %x data %x",bank, GI[CPU0][bank]);
	return GI[CPU0][bank];
}
