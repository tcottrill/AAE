...
int load_file(char* filename, unsigned char* buf, int size)
{
	...
	wrlog("loading save file");
	if (!fd) wrlog("Hi Score file for game:%s not found", driver[gamenum].name); return (0);
	fread(membuffer, size, 1, fd);
	fclose(fd);
	return (1);
}
...
{
	FILE* fd = NULL;
	wrlog("Writing save file");
	if (!(fd = fopen(filename, "wb"))) { wrlog("Saving Ram failed!"); return (0); }
