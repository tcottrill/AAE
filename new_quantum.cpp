...
void QSaveScore(void)
{
	...
	fp = fopen(temppath, "w");
	if (fp == NULL) { wrlog("Error - I couldn't write Quantum High Score"); }
...
int QLoadScore(void)
{
	...
	if (fp == NULL) { wrlog("Error - I couldn't READ Quantum High Score"); }
	if (fp != NULL)
	{
		do {
...
unsigned quantum_switches_r(unsigned address)
{
	...
	return input_port_0_r(0) | 0;
}


static int input_1_r(int offset)
{
	wrlog("Reading Input 1, offset %x", offset);
	return (readinputportbytag("DSW0") << (7 - (offset - POT0_C))) & 0x80;
}


static int input_2_r(int offset)
{
	wrlog("Reading Input 2, offset %x", offset);
...
int quantum_snd_read(unsigned address)
{
	if (address & 0x10)
...
{
	if (address & 0x10) /* A5 selects chip */
...
static struct POKEYinterface quantum_pokey_interface =
{
	2,			/* 2 chips */
	600000,
	200,	/* volume */
...
struct STARSCREAM_DATAREGION quantum_writebyte[] =
{
	...
	{ 0x018000, 0x01cfff, NULL, main_ram },
	{ 0x900000, 0x9001ff, NULL, nv_ram },
	{ 0x960000, 0x960001, MWA_NOP, NULL },
	{ -1, -1, NULL, NULL }
};
...
int init_quantum()
{
	...
	wrlog("one");
	byteswap(GI[CPU0], 0x13fff);
	memcpy(program_rom, GI[CPU0], 0x13fff);//0x14000
	//free(GI);
	...
	avg_init();
	wrlog("three");
	pokey_sh_start(&quantum_pokey_interface);
	//QLoadScore();
